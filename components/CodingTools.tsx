...
const CodeViewer = ({ files, instructions }: { files: { name: string; content: string; language: string }[]; instructions?: string }) => {
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select value={framework} onChange={e => setFramework(e.target.value as any)} className={inputClass}>
...
export const WebTemplateTool = () => {
    const [style, setStyle] = useState<'Minimal'|'Corporate'|'Startup'|'Portfolio'>('Startup');
    const [stack, setStack] = useState<'React/Tailwind'|'HTML/CSS'>('React/Tailwind');
    const [desc, setDesc] = useState('');
    const [result, setResult] = useState<any>(null);
    const [loading, setLoading] = useState(false);
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select value={style} onChange={e => setStyle(e.target.value as any)} className={inputClass}>
                        <option>Startup</option><option>Minimal</option><option>Corporate</option><option>Portfolio</option>
                    </select>
                </div>
                <div>
                    <label className={labelClass}>Tech Stack</label>
                    <select value={stack} onChange={e => setStack(e.target.value as any)} className={inputClass}>
