...
import { generateReelsScript, generateContentCalendar, generateMediaKitData, generateDesignConcept } from '../services/geminiService.ts';
...
export const CanvaTemplateTool: React.FC<ToolProps> = () => {
    ...
    const handleGenerate = async () => {
        ...
        setLoading(true);
        try {
            const res = await generateDesignConcept({ type: type as any, style, text, colors });
            setResult(res);
...
export const InstagramPackTool: React.FC<ToolProps> = () => {
...
export const ReelsScriptTool: React.FC<ToolProps> = () => {
    const [topic, setTopic] = useState('');
            ...
            {result && (
                <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 mt-4 space-y-4 max-h-96 overflow-y-auto">
                    ...
                    <div className="space-y-2">
                        {result.body.map((scene: string, i: number) => (
...
export const ThumbnailTool: React.FC<ToolProps> = () => {
    return <CanvaTemplateTool />; // Reuse Layout logic for MVP
};

// 5. Content Calendar
export const ContentCalendarTool: React.FC<ToolProps> = () => {
...
// 6. Media Kit
export const MediaKitTool: React.FC<ToolProps> = () => {
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {result.collaborationIdeas.map((idea: string, i: number) => (
