...
const DownloadButton = ({ content, filename, title }: { content: string, filename: string, title?: string }) => {
...
export const CheatSheetTool: React.FC<ToolProps> = () => {
...
// --- 2. Study Notes Tool ---
export const StudyNotesTool: React.FC<ToolProps> = () => {
    const [inputType, setInputType] = useState<'text' | 'file'>('text');
    const [rawContent, setRawContent] = useState('');
    const [fileData, setFileData] = useState<{ name: string, data: string, mimeType: string } | null>(null);
    const [result, setResult] = useState('');
...
// --- 3. Exam Guide Tool ---
export const ExamGuideTool: React.FC<ToolProps> = () => {
    const [inputType, setInputType] = useState<'text' | 'file'>('text');
    const [rawContent, setRawContent] = useState('');
    const [fileData, setFileData] = useState<{ name: string, data: string, mimeType: string } | null>(null);
    const [result, setResult] = useState('');
    ...
    const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const reader = new FileReader();
        reader.onload = (evt) => { const base64 = (evt.target?.result as string).split(',')[1]; setFileData({ name: file.name, mimeType: file.type, data: base64 }); };
        reader.readAsDataURL(file);
    };
...
// --- 4. Question Bank ---
export const QuestionBankTool: React.FC<ToolProps> = () => {
...
export const AssignmentTemplateTool: React.FC<ToolProps> = () => {
...
export const FlashcardTool: React.FC<ToolProps> = () => {
    const [topic, setTopic] = useState('');
