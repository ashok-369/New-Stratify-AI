...
import { generateLinkedInBios } from '../services/geminiService.ts';
import { Icon } from './Icon.tsx';
import { LinkedInParams } from '../types.ts';
...
export const LinkedInBioTool = () => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const results = await generateLinkedInBios(params as LinkedInParams);
            setBios(results);
        } catch (err: any) {
                                ...
                                {bios.map((bio, index) => (
                                    <div key={index} className="bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700 shadow-md hover:shadow-lg transition-all p-6 relative group">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {bio.hashtags.map((tag: string, i: number) => (
