...
export const NotionGenerator: React.FC<ToolProps> = () => {
...
export const PlannerBuilder: React.FC<ToolProps> = () => {
    ...
    const cleanText = (text: string) => text.replace(/\*\*/g, '').replace(/^[\*\-]\s*/gm, '').trim();
    ...
    const downloadPlanPDF = () => { 
        ...
        const margin = 20; 
        let y = 20; 
        const checkPageBreak = (needed: number) => { if (y + needed > 280) { doc.addPage(); y = 20; } }; 
                ...
                <div> 
                    ...
                    <div className="flex flex-wrap gap-2"> 
                        {Object.keys(config.sections).map((sec) => ( 
                            <button key={sec} onClick={() => toggleSection(sec)} className={`px-4 py-2 rounded-full text-xs font-bold border transition-all ${ /* @ts-ignore */ config.sections[sec] ? 'bg-[#001F3F] text-white border-[#001F3F]' : 'bg-white text-gray-500 border-gray-200 hover:border-gray-300'}`}>{sec.charAt(0).toUpperCase() + sec.slice(1)}</button> 
                        ))} 
                    </div> 
                </div> 
...
export const HabitTracker: React.FC<ToolProps> = () => {
    const [preferences, setPreferences] = useState(''); 
    const [result, setResult] = useState(''); 
    const [loading, setLoading] = useState(false); 
    const [viewMode, setViewMode] = useState<'doc' | 'raw'>('doc');
    ...
    const renderDocumentPreview = (markdown: string) => { 
    ...
    const downloadPDF = () => { 
        ...
        let y = 20; 
        const checkPageBreak = (needed: number) => { if (y + needed > pageHeight - margin) { doc.addPage(); y = 20; } }; 
...
export const FinanceTracker: React.FC<ToolProps> = () => {
    ...
    const updateExpense = (idx: number, field: string, val: string) => { const newExp = [...expenses]; /* @ts-ignore */ newExp[idx][field] = val; setExpenses(newExp); }; 
    ...
    const renderDocumentPreview = (markdown: string) => { 
        if (!markdown) return null; 
        const lines = markdown.split('\n'); 
        let inTable = false; let tableHeader: string[] = []; let tableRows: string[][] = []; 
        const renderedElements: React.ReactNode[] = []; 
    ...
    const downloadAIReport = () => { 
        ...
        const { jsPDF } = window.jspdf; 
        const doc = new jsPDF(); 
        const pageWidth = doc.internal.pageSize.getWidth(); const pageHeight = doc.internal.pageSize.getHeight(); const margin = 20; const maxLineWidth = pageWidth - (margin * 2); let y = 20; const checkPageBreak = (needed: number) => { if (y + needed > pageHeight - margin) { doc.addPage(); y = 20; } }; 
        ...
        const cleanForPdf = (text: string) => { if (!text) return ""; return text.replace(/(&lt;br&gt;|<br\s*\/?>|\\n)/gi, '\n').replace(/[\u2018\u2019]/g, "'").replace(/[\u201C\u201D]/g, '"').replace(/[\u2013\u2014]/g, '-').replace(/\*\*/g, '').replace(/__/g, '').replace(/\*/g, '').replace(/[^\x20-\x7E\n\r\t]/g, '').trim(); }; 
        const lines = aiResult.split('\n'); 
        let inTable = false; let tableData: string[][] = []; 
...
export const StudyPlanner: React.FC<ToolProps> = () => {
    // Modes
    const [mode, setMode] = useState<'ai' | 'manual'>('ai');
    const [viewMode, setViewMode] = useState<'doc' | 'raw'>('doc');
...
export const MeetingNotesGenerator: React.FC<ToolProps> = () => {
...
export const TimeBlocker: React.FC<ToolProps> = () => {
    ...
    // Use the same renderer logic as StudyPlanner
    const renderDocumentPreview = (markdown: string) => {
        if (!markdown) return null;
        const lines = markdown.split('\n');
        let inTable = false;
