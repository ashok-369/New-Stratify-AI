...
import { generateResume } from '../services/geminiService.ts';
import { Icon } from './Icon.tsx';
import { ResumeParams } from '../types.ts';
...
export const ResumeBuilderTool = () => {
    ...
    const handleSubmit = async () => {
        ...
        setError(null);
        try {
            const resume = await generateResume(payload as ResumeParams);
            setResult(resume);
        } catch (error: any) {
    ...
    const downloadPDF = () => {
        ...
        if (selectedTemplate === 'executive') {
            ...
            if (result.contactInfo.linkedin) contactLine += ` | LinkedIn`;
                ...
                {isExecutive ? (
                    <div className="bg-navy text-white p-12 mb-8">
                        ...
                        <div className="text-sm opacity-80 flex gap-4">
                            ...
                            {result.contactInfo.linkedin && <>|<span>LinkedIn</span></>}
                        </div>
                    </div>
                ) : (
