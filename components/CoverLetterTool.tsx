...
import { CoverLetterParams } from '../types.ts';
...
export const CoverLetterTool = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const text = await generateCoverLetter(params as CoverLetterParams);
  ...
  const handleExportPDF = () => {
    ...
    // --- Helper: Draw Icons ---
    const drawIcon = (type: string, x: number, y: number, color = [13, 148, 136]) => {
    ...
    if (selectedTemplate === 'modern') {
        ...
        const addLine = (icon: string, txt: string) => {
            if(!txt) return;
        ...
        let cx = 120;
        const addInline = (txt: string) => {
            if(!txt) return;
            doc.text(`•  ${txt}`, cx, 22);
            cx += (doc.getTextWidth(`•  ${txt}`) + 5);
        }
        ...
        doc.setFont("helvetica", "normal");
        let cy = 20;
        [params.email, params.phone, params.linkedin, params.address].filter(Boolean).forEach((txt: string) => {
  ...
  const FormatButton = ({ cmd, icon, label, onClick }: any) => (
    <button
        onMouseDown={(e) => {
            e.preventDefault(); 
            if (onClick) onClick();
